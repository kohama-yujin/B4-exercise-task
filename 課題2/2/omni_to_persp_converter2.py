...
omni_height, omni_width, channels = omni_img.shape[:3]
print("全方位画像のサイズ")
print("width, height")
print("({}, {})\n".format(omni_width, omni_height))

# 透視投影画像サイズの計算
if not("persp_width" in locals()):
    persp_width = int(2 * np.tan(np.radians(horizontal_fov / 2)) * (omni_width / (2 * np.pi)))
    persp_height = int(2 * np.tan(np.radians(vertical_fov / 2)) * (omni_height / np.pi))
print("透視投影画像のサイズ")
print("width, height")
...
if not("horizontal_fov" in locals()):
    horizontal_fov = np.degrees(2 * np.arctan2(np.pi * persp_width, omni_width))
    vertical_fov = np.degrees(2 * np.arctan2(np.pi * persp_height, (2 * omni_height)))
print("透視投影画像の画角")
...
# 回転角度の計算
theta_eye = np.degrees((u - omni_width / 2) * (2 * np.pi / omni_width))
phi_eye = np.degrees((omni_height / 2 - v) * (np.pi / omni_height))
print("視線回転角度")
print("水平θ, 垂直φ, 光軸ψ")
print("({}, {}, {})".format(round(theta_eye, 1), round(phi_eye, 1), round(psi_eye, 1)))
...
for j in range(persp_height):
    for i in range(persp_width):
        ...
        # 全方位画像の座標系変換
        u = (theta + np.pi) * (omni_width / (2 * np.pi))
        v = (np.pi / 2 - phi) * (omni_height / np.pi)
        # 透視投影にコピー
...
if path[0] == path_omni_img:
    path_persp_img = "({}_{}_{})_({}_{})_persp.jpg".format(round(theta_eye, 1), round(phi_eye, 1), round(psi_eye, 1), round(horizontal_fov, 1), round(vertical_fov, 1))
else:     
    path_persp_img = path[0] + "/({}_{}_{})_({}_{})_persp.jpg".format(round(theta_eye, 1), round(phi_eye, 1), round(psi_eye, 1), round(horizontal_fov, 1), round(vertical_fov, 1))
'''

path_persp_img = "output/({}_{}_{})_({}_{})_persp.jpg".format(round(theta_eye, 1), round(phi_eye, 1), round(psi_eye, 1), round(horizontal_fov, 1), round(vertical_fov, 1))
